create table Product(maker char(20) not null,model number not null ,type char(20) not null,primary key(model));
create table PC(model number not null,speed number not null,ram number not null,hd number not null,cd char(20) not null,price number not null,primary key(model));
create table Laptop(model number not null,speed number not null,ram number not null,hd number not null,screen number not null,price number not null,primary key(model));
create table Printer(model number not null,color char(20) not null,type char(20) not null,price number not null,primary key(model));
...
insert into PC values(1006,200,16,3.1,'8x',2099);
...
select model,speed,hd from PC where (cd='x6' or cd='x8') and price<2000;
select speed,marker from Laptop,Product where Laptop.model = Product.model and hd>=1;
