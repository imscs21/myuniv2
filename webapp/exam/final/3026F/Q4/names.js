...
function loadBabies(ajax) {
	var babies = ajax.responseXML.firstChild;
	var babies = babies.getElementsByTagName("name");
	for (var i = 0; i < babies.length; i++) {
		var option = $(document.createElement("option"));
		option.innerHTML = option.value = babies[i].innerHTML;
		$("allnames").appendChild(option);
	}
	$("allnames").disabled = false;
	
	/*
	var babies = ajax.responseText.split(" ");
	console.log(babies);
	for (var i = 0; i < babies.length; i++) {
		var option = $(document.createElement("option"));
		option.innerHTML = option.value = babies[i];
		$("allnames").appendChild(option);
	}
	$("allnames").disabled = false;
	*/
...
function displayList(ajax) {
	...
	var year = 1890;
	var ranks = JSON.parse(ajax.responseText).rankings; //ajax.responseText.split(" ");
